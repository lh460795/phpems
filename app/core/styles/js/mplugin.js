jQuery.extend({
	...
	'ajaxGo':function(e){
		...
		{
			$('<div class="pages" id="'+o.attr("data-page")+'"></div>').appendTo($('#content')).hide();
		}
		if(o.attr('data-markPrepage') == 'no')$.markPrepage = false;
		submitAjax(par);
	},
	'setPreWebPage':function(p){
		if($.markPrepage)
		$.preWebPage.push(p);
		else
		$.markPrepage = true;
	...
	'goPrePage':function(){
		if($.currentPage != null && $.preWebPage.length >= 1)
		{
			...
			p1.animate({'marginLeft':($.pewidth * 1)+'px'},function(){
				p1.hide();
				p2.css('margin-left','0px').fadeIn();
				$.setCurrentPage(p2);
		...
		else if($.currentPage.attr('id') != $.indexPage)
		{
			...
			else
			{
				...
				$('<div class="pages" id="page1"></div>').appendTo($('#content')).hide();
				submitAjax(par);
			}
		}
	},
